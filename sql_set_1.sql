...
select name from albums order by name asc limit 1;

select name from albums where release_year = (select min(release_year) from albums);
...
FROM (
    SELECT 
        name, 
        release_year, 
        RANK() OVER (ORDER BY CASE WHEN release_year IS NULL THEN 1 ELSE 0 END, release_year) AS rnk
    FROM albums
) sub
where rnk =1;
...
insert into bands values (8,'Imagine Dragons');

select * from albums;

insert into albums values (19,'Natural', 2024, 8);
...
select avg(length) as average_lenght from songs;
